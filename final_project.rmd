...
# 1) Read full‑season MoneyPuck data
shots <- read_csv("Data/shots_2024.csv", show_col_types = FALSE)

# 2) Filter for Toronto 5×5 on‑goal shots
leaf_5v5 <- shots %>%
  filter(
    teamCode         == "TOR",
    homeSkatersOnIce == 5,
...
# 3) Define the 12 Toronto forwards of interest
forwards_list <- c(
...
# 4) Build & render the rink + forwards’ goals
leaf_plot <-
...
# save tier list image using base R method
png("leafs_tier_list.png", width = 12, height = 12, units = "in", res = 600)
tierlist(df)  # now title_cex will be used properly
dev.off()
